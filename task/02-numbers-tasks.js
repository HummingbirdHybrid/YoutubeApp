 ...
 *   (0,1) (1,2)     => 0
 */
function getAngleBetweenVectors(ex, ey, cx, cy) {
    var dy = ey - cy;
    var dx = ex - cx;
    var theta = Math.atan2(dy, dx); // range (-PI, PI]
    theta = Math.abs(theta*2);
...
function toNumber(value, def) {
    if({}.toString.call(value).slice(8, -1) != "Number" && {}.toString.call(value).slice(8, -1) != "String" || isNaN(value)) return def;
    return +value;
}
